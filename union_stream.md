...
## 2-2. 하나의 그림으로 보기 — "집 안의 수도 설비"

여러 비유를 따로 외우면 헷갈린다. **모든 것을 하나의 수도 설비(plumbing)** 그림 안에 넣어 보자. 등장인물은 단 하나, "수도 시스템"이고 나머지는 그 부품일 뿐이다.

```
[디스크 = 큰 저수조]──수도관(stream)──[중간 물탱크 = 버퍼]──수도꼭지(FILE*)──> 프로그램(컵)
                                                              ▲
                                                       노즐 위치 = 파일 위치 표시자
```
...
**① 수도관 자체 = 스트림 (왜 필요한가: 추상화)**
- 강(디스크)까지 매번 양동이 들고 가는 대신, 집까지 **수도관**을 깔고 꼭지만 튼다.
- 디스크에 직접 접근하는 복잡한 일은 OS가 처리하고, 우리는 표준 수도꼭지(`FILE*`)만 다룬다.
- 그래서 파일·키보드·네트워크가 뭐든 **물 받는 동작은 똑같다**.

**② 중간 물탱크 = 버퍼 (왜 필요한가: 효율)**
- 디스크는 느리다. 그래서 흘려보낸 물을 바로 저수조로 보내지 않고, **꼭지 뒤 중간 물탱크**에 모았다가 가득 차면 한꺼번에 보낸다.
- `printf` 했는데 화면에 바로 안 보이면 → 아직 **중간 탱크에 고여 있는 중**.
- `fflush()` = "지금 당장 **중간 탱크를 비워** 저수조로 보내!"
- `fclose()` = 꼭지를 잠그면서 탱크에 남은 물도 마저 흘려보냄.

**③ 노즐 위치 = 파일 위치 표시자 (어디서 받고 있나)**
- 수도관 어느 지점에서 물을 받고 있는지 가리키는 **노즐**이 있다. 한 모금 받을 때마다 노즐이 저절로 앞으로 간다.
- `ftell()` = 지금 노즐이 관의 몇 번째 지점에 있나 확인.
- `fseek()` = 노즐을 관의 원하는 지점으로 옮기기.
- `rewind()` = 노즐을 관 입구(맨 앞)로 되돌리기.
- **EOF** = 저수조 바닥, 더 나올 물이 없다는 신호 (End Of File).

> 즉 **수도관(흐름) · 중간 탱크(버퍼) · 노즐(위치)** 세 부품만 기억하면 된다. 댐·택배·책갈피처럼 서로 다른 세계가 아니라, **한 수도 설비의 세 부분**이다.
...
int main(void) {
    ...
    int ch;
    // fgetc는 한 글자씩, 저수조 바닥(EOF)까지 흐름을 받는다
...
> **왜 `int ch`인가?** `fgetc`는 정상 글자(0~255)뿐 아니라 "물 떨어짐 신호 EOF(보통 -1)"도 돌려준다. `char`로 받으면 EOF와 일반 글자를 구분 못 할 수 있어, 더 넓은 그릇인 `int`로 받는다. (저수조가 비었다는 신호를 담을 칸이 필요)
...
| `fseek`/`ftell`/`rewind` | 노즐 위치 | 위치 이동/확인 |
| `fflush` | 중간 탱크 비우기 | 버퍼 강제 출력 |
| `feof`/`ferror` | 바닥/고장 점검 | 상태 확인 |
...
**문제 B4 ★★** — `printf("로딩중...");` 다음에 5초간 다른 작업을 하는데, 화면에 `로딩중...`이 즉시 안 보이고 한참 뒤 나타났다. **왜 그런가**를 "중간 물탱크(버퍼)" 비유로 설명하고, **한 줄을 추가**해 즉시 보이게 고쳐라.

**문제 B5 ★★** — 파일을 한 줄씩 읽어 **줄 번호를 붙여** 화면에 출력하는 프로그램을 작성하라. (`fgets` 사용, 예: `1: 첫째 줄`)

**문제 B6 ★★★** — `fseek`, `ftell`을 사용해, 어떤 텍스트 파일의 **크기(바이트 수)**를 한 글자도 읽지 않고 구하는 함수 `long file_size(const char *name)`를 작성하라. (힌트: 노즐을 관 끝으로 보낸 뒤 위치를 읽기)
...
**B4.** `printf`는 출력을 **중간 물탱크(버퍼)**에 모아두고, 줄바꿈(`\n`)이 없으면 바로 흘려보내지 않는다. 그래서 화면에 즉시 안 나타난다. `fflush(stdout);`을 추가하면 "지금 당장 중간 탱크를 비워라" → 즉시 표시된다. (또는 문자열 끝에 `\n` 추가)
```c
printf("로딩중...");
fflush(stdout);   // 추가: 중간 탱크 강제로 비우기
```
...
long file_size(const char *name) {
    ...
    if (fp == NULL) return -1;
    fseek(fp, 0, SEEK_END);   // 노즐을 관 맨 끝으로
...
```
`SEEK_END`로 노즐을 관 끝까지 보낸 뒤 `ftell`로 그 위치(=바이트 수)를 읽는다. 한 글자도 실제로 읽지 않는다.
...
**파일 I/O 스트림 — "집 안의 수도 설비" 하나로 기억**
- **수도관(stream)**: 데이터는 통째가 아니라 **흐름**으로 오간다. `fopen`(꼭지 열기) → 읽기/쓰기 → `fclose`(잠그기). 열고 나면 **NULL 검사** 필수.
- **중간 물탱크(buffer)**: 효율 위해 모았다 보냄. `fflush`로 즉시 비우기, `fclose`가 마저 비움.
- **노즐(위치 표시자)**: `fseek`/`ftell`/`rewind`로 이동·확인, 저수조 바닥 신호는 `EOF`.
