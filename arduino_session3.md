...
**Serial Monitor 열기:**
1. Tinkercad에서 "Code" 버튼 옆 "Serial Monitor" 버튼 클릭
2. Arduino IDE에서: 도구 → 시리얼 모니터 (또는 Ctrl+Shift+M)

**기본 설정:**
- 보드레이트(Baud rate): 9600 (통신 속도)
- 반드시 코드의 `Serial.begin()`과 같은 값 사용
...
**배열 크기:**
```c
const int NUM_LEDS = 5;  // 배열 크기를 상수로 선언 (권장)

// 또는 계산으로 구할 수 있음:
// int size = sizeof(ledPins) / sizeof(ledPins[0]);
// sizeof()는 배열이 차지하는 메모리 크기를 반환
```
